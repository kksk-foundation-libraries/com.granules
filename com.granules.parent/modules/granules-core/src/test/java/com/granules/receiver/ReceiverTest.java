...
import com.granules.receiver.MessageReceiveFlow.MessageContext;
...
public class ReceiverTest {
	...
	public void test001() {
		...
		TestMessageReceiveFlow flow = new TestMessageReceiveFlow(ignite, "message", "offset", "processing");
		flow.subscribe(new Subscriber<MessageContext>() {
			...
			public void onNext(MessageContext t) {
				LOG.debug("key:[{}], value:[{}]", hex(t.message().getKey()), hex(t.message().getValue()));
				upstream.request(1);
			}
		...
		cursorProcessing.forEach(entry -> {
			...
			LOG.debug("offset key:[{}], offset:[{}]", hex(messageOffsetKey.getKey()), proc);
			counter.incrementAndGet();
		});
